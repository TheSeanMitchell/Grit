...
def permits_to_cards(permits, source_key="clv_permit"):
    ...
    for p in permits:
        ...
        if p.get("contractor_phone") and not c.get("contractor_phone"):
...
def harvest():
    ...
    _write_cards(config.CARDS_FILE, {
        "generated_at": dt.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"),
        "version": config.VERSION,
...
# ---- helpers --------------------------------------------------------------
# Drag-and-drop install safety (0.111): GitHub's web uploader rejects any single
# file > 25 MB. cards.json is generated stored data that grows with the warehouse,
# so the shipped copy is kept lean: drop the per-card raw blob and the recomputable
# field_confidence map, and trim timeline events to what the console renders. These
# are all regenerated on the next harvest/rebuild, so nothing is lost. See
# MANIFESTO ("Distribution & file-size protocol").
MAX_DRAGDROP_MB = 25
_SLIM_DROP = ("raw", "field_confidence")


def _slim_card(c):
    out = {k: v for k, v in c.items() if k not in _SLIM_DROP}
    tl = out.get("timeline")
    if isinstance(tl, list):
        out["timeline"] = [{"date": e.get("date"), "kind": e.get("kind"),
                            "description": e.get("description") or e.get("address")}
                           for e in tl]
    return out


def _write_cards(path, obj):
    """Write the console's cards file as COMPACT json (no indentation) and warn
    loudly if it approaches the 25 MB drag-and-drop ceiling."""
    with open(path, "w") as f:
        json.dump(obj, f, separators=(",", ":"), default=str)
    mb = os.path.getsize(path) / 1048576
    if mb > MAX_DRAGDROP_MB - 1:
        print(f"  WARNING: {path} is {mb:.1f} MB -- near the {MAX_DRAGDROP_MB} MB "
              f"drag-and-drop limit. It is regenerated by the harvest, so exclude it "
              f"from manual web uploads and let the GitHub Action commit it (git push "
              f"is not bound by the 25 MB web limit). See MANIFESTO file-size protocol.")
    return mb
def _write(path, obj):
    with open(path, "w") as f:
